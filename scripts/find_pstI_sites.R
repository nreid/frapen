...
# This R script finds pstI cut sites, flanking sites and writes them to bed files
...
seqs <- readDNAStringSet("pennsylvanica.fasta")

# find pstI sites
mp <- vmatchPattern("CTGCAG",seqs)
...
# write tables
write.table(mp3[,1:3],"../meta/pstI.bed",sep="\t",row.names=FALSE,col.names=FALSE,quote=FALSE)
write.table(mp3_1[,1:3],"../meta/pstI_off.bed",sep="\t",row.names=FALSE,col.names=FALSE,quote=FALSE)


# find the distances from pstI to mseI cut sites
...
# get closest upstream and downstream msei cut sites to pstI cut sites
